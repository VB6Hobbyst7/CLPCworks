...
#对未进入mysql数据库的个养销售记录表格记录进行汇总、清洗
...
def Acc_gen(raw_tab,path,a):
    ...
    for i in range(len(cust_list)):
        ...
        try:
            ...
            Acct_prs.to_excel("%saccount/%s-%s.xlsx" %(path,cust_temp,cust.get(cust_temp)))
    ...
    # 生成透视表
    if a==3 or a==4:
        city_pivot=pd.pivot_table(f,values=['performance'],index=[alter_para1],columns=['FUND_CLASS'],aggfunc=np.sum,margins=True)
    elif a==1 or a==2:
        city_pivot=pd.pivot_table(f,values=['performance'],index=[alter_para2],columns=['FUND_CLASS'],aggfunc=np.sum,margins=True)
    else:
    ...
    general_tab.to_excel("%s账户出现最大值统计列表.xlsx" %path)
