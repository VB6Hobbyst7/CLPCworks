...
data_rw=pd.read_excel(file_path+'/18383资金流水.xlsx',dtype={"证券代码":str},sheet_name="资金流水")
stockholdings=pd.read_excel(file_path+'/证券交易0411.xlsm',sheet_name='逐日资产统计')

data_rw['成交日期']=pd.to_datetime(data_rw['成交日期'],format='%Y%m%d')
...
test_date=stockholdings.iloc[-1,0]
stockholdings=stockholdings[stockholdings['日期']==test_date]
...
grand_pivot['盈亏合计']=grand_pivot.sum(axis=1)

fund_account=round(grand_pivot.sum(axis=0),2)
grand_pivot.loc['合计数']=grand_pivot.apply(lambda x:x.sum())
...
result=grand_pivot[grand_pivot['盈亏合计']!=0]
result.to_excel(file_path+'\资金决算分析.xlsx')
